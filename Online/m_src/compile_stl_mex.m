...
compile_obj__=1;
compile_mex__=1;
compile_simulink__=0;
    
src_mex = 'stl_eval_mex.cpp';
src_mex_all = 'stl_eval_mex_pw.cpp';
src_mex_cau = 'stl_causation.cpp';
src_mex_cau_mid = 'stl_causation_mid.cpp';
src_mex_cau_opt = 'stl_causation_opt.cpp';
...
if (compile_mex__)
    ...
    eval(cmd_exe)
    cmd_exe_all = sprintf('mex  %s %s %s%s %s -outdir %s', cxxflags, includes, prefix_m, src_mex_all , objs, bin_dir);
    eval(cmd_exe_all)
    cmd_exe_cau = sprintf('mex  %s %s %s%s %s -outdir %s', cxxflags, includes, prefix_m, src_mex_cau , objs, bin_dir);
    eval(cmd_exe_cau)
    cmd_exe_cau_mid = sprintf('mex  %s %s %s%s %s -outdir %s', cxxflags, includes, prefix_m, src_mex_cau_mid , objs, bin_dir);
    eval(cmd_exe_cau_mid)
    cmd_exe_cau_opt = sprintf('mex  %s %s %s%s %s -outdir %s', cxxflags, includes, prefix_m, src_mex_cau_opt , objs, bin_dir);
    eval(cmd_exe_cau_opt)
end
